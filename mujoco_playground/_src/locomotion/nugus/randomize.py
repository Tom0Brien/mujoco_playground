...
# ==============================================================================
"""Domain randomization for the Nugus environment."""
...
FLOOR_GEOM_ID = 0
TORSO_BODY_ID = 1
ANKLE_JOINT_IDS = np.array([[11, 12, 23, 24]])  # Adjusted for Nugus
...
def domain_randomize(model: mjx.Model, rng: jax.Array):
    ...
    def rand_dynamics(rng):
        ...
        # Adjust for Nugus joint count
        rng, key = jax.random.split(rng)
        frictionloss = model.dof_frictionloss[6:] * jax.random.uniform(
            key, shape=(model.dof_frictionloss.shape[0] - 6,), minval=0.9, maxval=1.1
        )
        ...
        armature = model.dof_armature[6:] * jax.random.uniform(
            key, shape=(model.dof_armature.shape[0] - 6,), minval=1.0, maxval=1.05
        )
        ...
        qpos0 = qpos0.at[7:].set(
            qpos0[7:]
            + jax.random.uniform(
                key, shape=(qpos0.shape[0] - 7,), minval=-0.05, maxval=0.05
            )
        )
        ...
        kd = model.dof_damping[6:] * jax.random.uniform(
            key, (model.dof_damping.shape[0] - 6,), minval=0.9, maxval=1.1
        )
