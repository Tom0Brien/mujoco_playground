...
def default_config() -> config_dict.ConfigDict:
    """Returns a default ConfigDict for the NUGUS Joystick task."""
    cfg = config_dict.create(
        ...
        noise_config=config_dict.create(
            level=1.0,  # Set to 0.0 to disable noise entirely.
        ...
        # Joystick command ranges (m/s, rad/s).
        lin_vel_x=[-0.5, 0.5],
        lin_vel_y=[-0.2, 0.2],
        ang_vel_yaw=[-0.4, 0.4],
    )
...
class Joystick(nugus_base.NugusEnv):
    """Tracks a joystick command for the NUGUS robot."""
    ...
    def __init__(
        ...
        """Initializes the NUGUS joystick environment.

        Args:
          task: Which environment/task to load. For example:
                'flat_terrain' or 'rough_terrain'.
          config: Base ConfigDict for environment hyperparams.
          config_overrides: Optional dictionary for overrides.
        """
        # Map the user task string to the appropriate XML:
        # e.g. "flat_terrain" => "scene_mjx_flat_terrain.xml"
        #     "rough_terrain" => "scene_mjx_rough_terrain.xml"
    ...
    def _post_init(self) -> None:
        """Finish setting up internal references after super init."""
        # Grab the 'home' keyframe to set default init pose.
        self._init_q = jp.array(self._mj_model.keyframe("home").qpos)
        self._default_pose = jp.array(self._mj_model.keyframe("home").qpos[7:])

        # The first 7 elements in qpos are the free joint (pos + quat).
        ...
        # Torso reference: "torso" is the root body in nugus.xml
        self._torso_body_id = self._mj_model.body("torso").id
        self._torso_mass = self._mj_model.body_subtreemass[self._torso_body_id]
        self._site_id = self._mj_model.site("imu").id  # IMU site for orientation etc.

        # The NUGUS foot sites are typically "left_foot" and "right_foot".
        ...
        # Floor geom is often named "floor" in scene XMLs.
        self._floor_geom_id = self._mj_model.geom("floor").id

        self._left_feet_geom_id = np.array(
            [
                self._mj_model.geom("l_foot1").id,
            ]
        )
        self._right_feet_geom_id = np.array(
            [
                self._mj_model.geom("r_foot1").id,
            ]
        )

        # We also track foot velocity sensor addresses if you have them (e.g. "left_foot_global_linvel").
        foot_linvel_sensor_adr = []
        for site in ["left_foot_global_linvel", "right_foot_global_linvel"]:
            sensor_id = self._mj_model.sensor(site).id
            adr = self._mj_model.sensor_adr[sensor_id]
            dim = self._mj_model.sensor_dim[sensor_id]
            foot_linvel_sensor_adr.append(range(adr, adr + dim))
        self._foot_linvel_sensor_adr = jp.array(foot_linvel_sensor_adr)

        # Example geoms for bounding box collisions, if needed:
        self._left_foot_box_geom_id = self._mj_model.geom("l_foot1").id
        self._right_foot_box_geom_id = self._mj_model.geom("r_foot1").id
    ...
    def _get_termination(self, data: mjx.Data) -> jax.Array:
        """Returns True if the episode terminates."""
